...
import { UIPanel, UIRow, UIElement, UIDiv, UIInput, UIButton, UIColor, UICheckbox, UIInteger, UITextArea, UIText, UINumber, UIBreak } from './libs/ui.js';
...
function SidebarObject(editor) {
	...
	// Save / Load button
	const saveLoadDiv = new UIDiv();
	objectUserDataRow.add(saveLoadDiv);
	saveLoadDiv.add(objectUserData);
	container.add(objectUserDataRow);

	const saveLoadRow = new UIRow();
	saveLoadDiv.add(saveLoadRow);

	const saveBtn = new UIButton("Save");
	saveLoadRow.add(saveBtn);
	saveLoadRow.add(new UIBreak().setWidth('5px'));
	saveBtn.onClick(function() {
		const object = editor.selected;
		if (object === null) return;

		const data = {};
		data.events = object['userData'] || {};

		try {
			const partsData = object?.children[0]?.children[0]?.children[1]?.children;
			if (Array.isArray(partsData)) {
				data.parts = {};
				for (const child of partsData) {
					const type = child.name;
					data.parts[type] = {};
					for (const part of child.children) {
						data.parts[type][part.name] = part.userData;
					}
				}
			}
			saveString(JSON.stringify(data), `${object.name}.udt`);
		} catch (e) {
			console.error(e);
		}
	});
	...
	loadBtn.onClick(function() {
		...
		fileInput.addEventListener('change', function() {
			...
			if (extension !== 'udt') return;

			const reader = new FileReader();
			reader.addEventListener('load', function(event) {
				const contents = event.target.result;
				const data = JSON.parse(contents, null, '  ');
				object.userData = data.events;
				...
				if (Array.isArray(partsData)) {
					for (const child of partsData) {
						const type = child.name;
						if (!data.parts[type]) continue;
						for (const part of child.children) {
							if (!data.parts[type][part.name]) continue;
							part.userData = data.parts[type][part.name];
						}
					}
				}
			});
